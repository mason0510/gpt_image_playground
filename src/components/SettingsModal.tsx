...
import {
  ...
  isBuiltInOpenAICompatibleProvider,
...
export default function SettingsModal() {
  ...
  const apiKeySource = getApiKeySource(activeProfile.apiKey)
  const limitedFreeSelected = apiKeySource === 'limited-free'
  const apiProxyChecked = activeProfileApiProxyEligible && (limitedFreeSelected ? apiProxyAvailable : activeProfile.apiProxy)
  const apiProxyEnabled = apiProxyAvailable && activeProfileApiProxyEligible && apiProxyChecked
  const apiUrlLockedByLimitedFree = apiProxyEnabled && limitedFreeSelected
  const canUseLimitedFreeKey = activeProviderIsOpenAICompatible && apiProxyAvailable
  const apiUrlDisplayValue = apiUrlLockedByLimitedFree ? '' : activeProfile.baseUrl
  const apiUrlPlaceholder = apiUrlLockedByLimitedFree
    ? '由部署端代理决定（实际请求 /api-proxy/v1）'
    : activeProfile.provider === 'fal'
      ? 'https://your-fal-proxy.example.com'
      : DEFAULT_SETTINGS.baseUrl
  const customProviderIds = draft.customProviders.map((provider) => provider.id)
  const defaultProviderOrder = ['openai', 'sublb', ...customProviderIds]
  const providerOrderSource = draft.providerOrder || defaultProviderOrder
  const providerOrder = providerOrderSource.filter((providerId, index, list) => {
    if (providerId === 'fal') return false
    if (providerId === 'openai' || providerId === 'sublb') return list.indexOf(providerId) === index
    return customProviderIds.includes(providerId) && list.indexOf(providerId) === index
  })

  const unorderedProviderOptions = [
    { label: 'OpenAI 兼容接口', value: 'openai', draggable: true },
    { label: 'SubLB', value: 'sublb', draggable: true },
  ...
  const getDefaultModelForMode = (_apiMode?: AppSettings['apiMode']) => DEFAULT_IMAGES_MODEL
    ...
    const nextDraft = normalizeSettings({
      ...
      profiles: displaySettings.profiles.map((profile) => ({
        ...
        apiProxy: isProfileApiProxyEligible(displaySettings, profile) && apiProxyAvailable
          ? (isLimitedFreeApiKey(profile.apiKey) ? true : profile.apiProxy)
          : false,
      })),
    })
    setDraft(nextDraft)
  ...
  const commitSettings = (nextDraft: AppSettings) => {
    const normalizedProfiles = nextDraft.profiles.map((profile) => {
      const nextApiProxy = isProfileApiProxyEligible(nextDraft, profile) && apiProxyAvailable ? (isLimitedFreeApiKey(profile.apiKey) ? true : profile.apiProxy) : false
      ...
      return {
        ...
        apiMode: 'images',
        codexCli: isBuiltInOpenAICompatibleProvider(profile.provider) ? profile.codexCli : false,
        streamImages: isBuiltInOpenAICompatibleProvider(profile.provider) ? profile.streamImages : false,
        responseFormatB64Json: false,
        streamPartialImages: isBuiltInOpenAICompatibleProvider(profile.provider) ? normalizeStreamPartialImages(profile.streamPartialImages) : DEFAULT_STREAM_PARTIAL_IMAGES,
      }
    })
  ...
  const handleApiKeySourceChange = (value: string) => {
    if (value === 'limited-free') {
      updateActiveProfile({
        apiKey: LIMITED_FREE_API_KEY_SENTINEL,
        apiProxy: activeProfileApiProxyEligible && apiProxyAvailable ? true : activeProfile.apiProxy,
      }, true)
      return
    }
    updateActiveProfile({
      apiKey: apiKeySource === 'limited-free' ? '' : activeProfile.apiKey,
      apiProxy: apiKeySource === 'limited-free' ? false : activeProfile.apiProxy,
      baseUrl: activeProfile.baseUrl.trim() ? activeProfile.baseUrl : DEFAULT_SETTINGS.baseUrl,
    }, true)
  }
  ...
  const duplicateActiveProfile = () => {
    ...
    const profile: ApiProfile = {
      ...activeProfile,
      id: newId(isBuiltInOpenAICompatibleProvider(activeProfile.provider) ? activeProfile.provider : 'profile'),
  ...
  const handleProviderReorder = (sourceValue: string | number, targetValue: string | number, position: 'before' | 'after' | null) => {
    const currentOrder = providerOrder
        ...
        <div className="flex flex-1 min-h-0 flex-col sm:flex-row">
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'api' && (
              ...
              {activeProviderUsesApiUrl && (
                <label className="block">
                  ...
                  <input
                    ...
                    type="text"
                    disabled={apiUrlLockedByLimitedFree}
                    placeholder={apiUrlPlaceholder}
                    className={`w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50 ${apiUrlLockedByLimitedFree ? 'opacity-50 cursor-not-allowed' : ''}`}
                  />
                  <div data-selectable-text className="mt-1.5 min-h-[22px] flex items-center text-xs text-gray-500 dark:text-gray-500">
                    {apiUrlLockedByLimitedFree ? (
                      <span className="text-yellow-600 dark:text-yellow-500">已锁定同源代理：浏览器实际请求 /api-proxy/v1/...，不会直连上方 URL；请只填写 API Key。</span>
                    ) : activeProfile.provider === 'fal' ? (
                      <span>旧版兼容配置不再内置默认 URL；请填写你自己的自定义 fal API / 代理地址。</span>
                    ) : (
                ...
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (!limitedFreeSelected) updateActiveProfile({ apiProxy: !activeProfile.apiProxy }, true)
                      }}
                      disabled={limitedFreeSelected}
                      className={`relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${apiProxyChecked ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'} ${limitedFreeSelected ? 'cursor-not-allowed opacity-70' : ''}`}
                      role="switch"
              ...
              {/* 6. 调用格式 */}
              {isBuiltInOpenAICompatibleProvider(activeProfile.provider) && (
                <div className="block rounded-xl border border-gray-200/70 bg-gray-50/70 px-3 py-2.5 text-xs leading-5 text-gray-600 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-400">
                  调用格式固定为 <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">Images API (/v1/images/generations)</code>。
                  可继续自定义 API 地址、Key 和模型 ID；不再提供 Responses API 作为普通生图格式。
                </div>
              )}
              ...
              <div className="flex h-full min-h-[300px] flex-col items-center justify-center pb-8 px-6">
                ...
                */}
                ...
                >
                  ...
                  <h4 className="text-[17px] font-bold text-gray-800 dark:text-gray-100">Imagination Space</h4>
